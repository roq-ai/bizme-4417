...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createCompany() {
    ...
    const body = { ...req.body };
    if (body?.job_opportunity?.length > 0) {
      const create_job_opportunity = body.job_opportunity;
      body.job_opportunity = {
        create: create_job_opportunity,
      };
    } else {
      delete body.job_opportunity;
    }
